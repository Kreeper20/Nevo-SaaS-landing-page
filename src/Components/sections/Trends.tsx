export const Trends = () => {
    ...
    return (
        <>
            <section id="features" className="border-t border-b border-border/90 bg-surface/35">
                <div className="mx-3 mt-16 mb-16 md:mx-20 lg:mx-32 2xl:mx-80 grid items-start gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.25fr)] lg:gap-18">
                    <aside className="bg-surface border order-2 md:order-1 border-border shadow-lg shadow-black rounded-2xl overflow-hidden w-full max-w-2xl">
                        ...
                        <div className="px-6 pb-6 flex flex-col gap-1">
                            {trends.map((trend) => (
                                <div key={trend.rank} className="flex items-center gap-4 py-3 border-b border-border last:border-none">
                                    <span className="text-neutral-600 text-sm w-4 shrink-0">{trend.rank}</span>
                                    <span className="text-white text-sm font-medium w-28 shrink-0">{trend.topic}</span>
                                    <div className="flex-1 h-2 bg-neutral-800 rounded-full overflow-hidden">
                                        <div className={`h-full rounded-full ${trend.color}`} style={{ width: trend.width }} />
                                    </div>
                                    <span className={`text-sm font-bold w-6 text-right ${trend.countColor}`}>{trend.count}</span>
                                    <span className="text-base w-5 text-center">{trend.arrow}</span>
                                </div>
                            ))}
                        </div>
                    </aside>
                    ...
                    <div className="lg:pt-2 order-1 lg:order-2">
                        ...
                        <ul className="list-disc pl-5 mt-2 text-sm text-muted">
                            <li>Tracks 100+ topics simultaneously across every source</li>
